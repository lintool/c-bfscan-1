...
extern void init_tf_impact_score_8bits(char *data_path, int b);
int num_topics;
int num_docs;
long total_terms;
...
int main(int argc, const char* argv[]) {
  ...
  int b = 8;
  init_tf_impact_score_8bits(argv[1], b);
  int cnt = 0;
  for (cnt = 0; cnt < 3; cnt ++) {
  int n, i;
  threadpool thpool;
  ...
  for (n = 0; n < num_topics; n ++) {
    ...
    time_spent = (end.tv_sec - begin.tv_sec) * 1000;
    time_spent += (end.tv_nsec - begin.tv_nsec) / 1000000.0;
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          ...
          scores[pos] = scores[pos] + impact_score_8bits_padding[cur + map[bits]];
        }
      }
      ...
      for (j = 0; j < doclengths_ordered_padding[i]; j ++) {
        for (t = 2; t < 2 + topics[n][1]; t ++) {
          if (collection_tf_padding[base + j] == topics[n][t]) {
            score += impact_score_8bits_padding[base + j];
          }
        }
      }
