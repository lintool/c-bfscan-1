...
public class RunQueries {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int count = 1; count <= N; count ++) {
      ...
      // out.println("time per query = " + (System.currentTimeMillis() - start) / 49 + " ms");
      // out.println("throughput = " + 49 / ((double)(System.currentTimeMillis() - start) / 1000) + " qps");
      if (count != 1) {
        total = total + ((System.currentTimeMillis() - start));
      }
    }
    // out.println("time per query = " + total / (N - 1) / (queryCount / N) + " ms");
    // out.println("throughput = " + (queryCount / N) / ( total / (N - 1) / 1000) + " qps");
    out.println(total / (N - 1) / (queryCount / N));
    reader.close();
    out.close();
  }
}
