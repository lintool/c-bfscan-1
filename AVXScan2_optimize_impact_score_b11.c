...
extern void init_tf_impact_score_32bits(char *data_path, int b);
int num_topics;
int num_docs;
int total_terms;
...
int main(int argc, const char* argv[]) {
  ...
  int cnt;
  for (cnt = 0; cnt < 3; cnt ++) {
  init_tf_impact_score_32bits(argv[1], b);
  ...
  for (n = 0; n < num_topics; n ++) {
    ...
    if (topics[n][1] == 1) {
      ...
      for (i = 0; i + jump < num_docs; i += jump) {
        ...
        for (j = 0; j < indexsum; j += 8) {
          ...
          if (bits != 0) {
            score_pos = cur + map[bits];
            scores[pos] = scores[pos] + impact_score_32bits_padding[score_pos];
          }
        }
        ...
        for (j = 0; j < doclengths_ordered_padding[i]; j ++) {
          for (t = 2; t < 2 + topics[n][1]; t ++) {
            if (collection_tf_padding[base + j] == topics[n][t]) {
              score += impact_score_32bits_padding[base + j];
            }
          }
        }
    ...
    while (heap_delmin(&h, (void**)&min_key, (void**)&min_val)) {
      printf("MB%02d Q0 %ld %d %f AVXScan2_optimize_impact_score_b11\n", (n + 1), tweetids[*min_val], rank, *min_key);
      rank --;
    }
