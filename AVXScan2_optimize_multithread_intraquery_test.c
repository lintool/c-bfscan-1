...
int search(struct arg_struct *arg) {
  ...
  if (topics[n][1] == 1) {
    ...
    float part2 = MU * (cf[topics[n][2]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          ...
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part2) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part2 = MU * (cf[topics[n][2]] + 1) / (total_terms + 1);
    float part3 = MU * (cf[topics[n][3]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part3) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part2 = MU * (cf[topics[n][2]] + 1) / (total_terms + 1);
    float part3 = MU * (cf[topics[n][3]] + 1) / (total_terms + 1);
    float part4 = MU * (cf[topics[n][4]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part4) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part3 = MU * (cf[topics[n][3]] + 1) / (total_terms + 1);
    float part4 = MU * (cf[topics[n][4]] + 1) / (total_terms + 1);
    float part5 = MU * (cf[topics[n][5]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part5) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part4 = MU * (cf[topics[n][4]] + 1) / (total_terms + 1);
    float part5 = MU * (cf[topics[n][5]] + 1) / (total_terms + 1);
    float part6 = MU * (cf[topics[n][6]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part6) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part5 = MU * (cf[topics[n][5]] + 1) / (total_terms + 1);
    float part6 = MU * (cf[topics[n][6]] + 1) / (total_terms + 1);
    float part7 = MU * (cf[topics[n][7]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part7) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part6 = MU * (cf[topics[n][6]] + 1) / (total_terms + 1);
    float part7 = MU * (cf[topics[n][7]] + 1) / (total_terms + 1);
    float part8 = MU * (cf[topics[n][8]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part8) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part7 = MU * (cf[topics[n][7]] + 1) / (total_terms + 1);
    float part8 = MU * (cf[topics[n][8]] + 1) / (total_terms + 1);
    float part9 = MU * (cf[topics[n][9]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part9) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part8 = MU * (cf[topics[n][8]] + 1) / (total_terms + 1);
    float part9 = MU * (cf[topics[n][9]] + 1) / (total_terms + 1);
    float part10 = MU * (cf[topics[n][10]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part10) * (MU / (doclengths[i + pos] + MU)));
        }
      }
    ...
    float part9 = MU * (cf[topics[n][9]] + 1) / (total_terms + 1);
    float part10 = MU * (cf[topics[n][10]] + 1) / (total_terms + 1);
    float part11 = MU * (cf[topics[n][11]] + 1) / (total_terms + 1);
    ...
    for (i = start; i + jump < end; i += jump) {
      ...
      for (j = 0; j < indexsum; j += 8) {
        ...
        if (bits != 0) {
          int pos = (doc_pos[cur >> 3] + off) % DOCS_BLOCK_SIZE;
          scores[pos] = scores[pos] + log((1 + tf_padding[cur + map[bits]] / part11) * (MU / (doclengths[i + pos] + MU)));
        }
      }
      ...
      for (j = 0; j < doclengths_ordered_padding[i]; j ++) {
        for (t = 2; t < 2 + topics[n][1]; t ++) {
          if (collection_tf_padding[base + j] == topics[n][t]) {
            score = score + log((1 + tf_padding[base + j] / (MU * (cf[topics[n][t]] + 1) / (total_terms + 1))) * (MU / (doclengths[i] + MU)));
          }
        }
      }
